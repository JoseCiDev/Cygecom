@push('styles')
    <style>
        #modal-supplies-service-info .modal-body {
            padding-bottom: 50px;
        }
...
<div class="modal fade" id="modal-supplies-service-info" tabindex="-1" role="dialog" aria-labelledby="modal-label" aria-hidden="true">
    <div class="modal-dialog modal-fullscreen">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="modal-label">
                    <strong>Solicitação de serviço pontual - Nº <span class="modal-request-id"></span></strong>
                </h4>
            ...
            $suppliesProductInfo.on('show.bs.modal', function(event) {
                ...
                $.get({
                    ...
                    success: (data) => {
                        ...
                        const supplier = {
                            corporate_name: service.supplier?.corporate_name,
                            cpf_cnpj: service.supplier?.cpf_cnpj,
                            market_type: service.supplier?.market_type
                        };
                        ...
                        Object.values(content).forEach((item, index) => {
                            const options = {
                                basicInfo: () => {
                                    const $newList = $(`
                                        <ul class="list-group half-width">
                                            ...
                                            <li class="list-group-item"><strong>Valor total:</strong> R$ ${numberFormat(item.price)}</li>
                                ...
                                supplier: () => {
                                    const $newList = $(`
                                        <ul class="list-group half-width">
                                            <h5 class="mb-1"><i class="fa-solid fa-building"></i> <strong>Fornecedor</strong></h5>
